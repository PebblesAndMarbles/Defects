...
class DefectDataProcessor(ProcessorBase):
    ...
    def add_basic_columns(self, dt: pd.DataFrame) -> pd.DataFrame:
        ...
        # NEW: Create SUM_EDI column (if BEEP_EDI and SMP_EDI are present)
        if 'BEEP_EDI' in dt.columns and 'SMP_EDI' in dt.columns:
            self.logger.info("Creating SUM_EDI column from BEEP_EDI + SMP_EDI...")
            dt['SUM_EDI'] = pd.to_numeric(dt['BEEP_EDI'], errors='coerce').fillna(0) + \
                           pd.to_numeric(dt['SMP_EDI'], errors='coerce').fillna(0)
        else:
            if 'BEEP_EDI' not in dt.columns:
                self.logger.info("BEEP_EDI column not found, skipping SUM_EDI derivation")
            if 'SMP_EDI' not in dt.columns:
        ...
        # NEW: Create ColumnManager calls for EDI derived columns (if SUM_EDI exists)
        if 'SUM_EDI' in dt.columns:
            self.logger.info("Creating ColumnManager for EDI derived columns...")
            dt = ColumnManager.create_ncdd_derived_columns(dt, 'SUM_EDI', 0.02, 'EDI')
            dt = ColumnManager.create_ncdd_derived_columns(dt, 'BEEP_EDI', 0.0094, 'BEEP_EDI')
            dt = ColumnManager.create_ncdd_derived_columns(dt, 'SMP_EDI', 0.013, 'SMP_EDI')
    ...
    def _finalize_dataframe(self, dt: pd.DataFrame) -> pd.DataFrame:
        ...
        # PHASE 2B FIX: Delete unwanted columns that should not be generated by pipeline
        unwanted_cols = [
            'STATUS_EDI', 'CLASS_EDI', 'ZERO_EDI',
            'STATUS_BEEP_EDI', 'CLASS_BEEP_EDI', 'ZERO_BEEP_EDI',
            'STATUS_SMP_EDI', 'CLASS_SMP_EDI', 'ZERO_SMP_EDI'
        ]
        cols_to_drop = [col for col in unwanted_cols if col in dt.columns]
        if cols_to_drop:
            self.logger.info(f"Removing {len(cols_to_drop)} unwanted columns: {cols_to_drop}")
