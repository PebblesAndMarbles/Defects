...
def _check_recent_raw_inputs(max_age_days: int) -> List[str]:
    """Check that rolling NCDD_EDI CSV files exist and are recent.
    
    PHASE 2B: Now checking for rolling 10-day NCDD+EDI files instead of separate NCDD CSVs.
    These files contain both metrics and are generated by scheduled JMP query.
    """
    checks = []
    now = pd.Timestamp.now()
    for label, path in {
        "m5_ncdd_edi_csv": PIPELINE_PATHS.m5_ncdd_edi_csv,
        "m6_ncdd_edi_csv": PIPELINE_PATHS.m6_ncdd_edi_csv,
    }.items():
        if not path.exists():
...
def _require_fresh_raw_inputs(max_age_days: int) -> None:
    ...
    if stale_or_missing:
        raise RuntimeError(
            "Raw NCDD_EDI inputs are missing or stale. Run the 10-day rolling JSL query first. "
...
def _run_weekly_zero_rate_aggregator() -> None:
    """
    Run the weekly zero-rate aggregator (conditional on Sunday).
    
    Generates weekly metrics from entire production CSV.
    Only executes on Sunday (first day of new week) to ensure complete week data.
    """
    import subprocess
    
    # Check if today is Sunday (weekday() == 6)
    if datetime.now().weekday() != 6:
        LOGGER.info("[weekly_zero_rate] Skipping - not Sunday (runs only on Sundays)")
        return
    
    script_path = CURRENT_DIR / "WEEKLY_ZERO_RATE_AGGREGATOR.py"
    input_csv = PIPELINE_PATHS.extended_output_csv
    output_dir = PIPELINE_PATHS.benchmark_outputs_dir
    
    if not script_path.exists():
        raise FileNotFoundError(f"Weekly aggregator script not found: {script_path}")
    
    if not input_csv.exists():
        raise FileNotFoundError(f"Extended output CSV not found: {input_csv}")
    
    # Create output directory
    output_dir.mkdir(parents=True, exist_ok=True)
    
    LOGGER.info(
        "[weekly_zero_rate] Running aggregator: input=%s output_dir=%s",
        input_csv,
        output_dir
    )
    ...
    result = subprocess.run(
        [sys_module.executable, str(script_path), str(input_csv), str(output_dir)],
        capture_output=True,
        text=True,
    ...
    if result.stdout:
        for line in result.stdout.split('\n'):
            if line.strip():
                LOGGER.info("[weekly_zero_rate] %s", line)
    
    if result.returncode != 0:
        error_msg = result.stderr or "Unknown error"
        LOGGER.warning("[weekly_zero_rate] Script exited with code %s: %s", result.returncode, error_msg)
        # Don't raise - this is optional enrichment
    else:
