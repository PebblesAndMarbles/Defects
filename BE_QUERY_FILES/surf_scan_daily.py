...
_SS_7DAY_REPORTS_SCRIPT = Path(__file__).resolve().parent.parent / "html" / "SS_INLINE_PRODUCTION_SUBENTITY_REPORTS_7DAY.py"
...
def _run_ss_7day_reports() -> None:
    """Regenerate the 7-day per-chamber SS HTML fleet reports after a successful pipeline run."""
    spec = importlib.util.spec_from_file_location("_ss_inline_reports_fleet_7day", _SS_7DAY_REPORTS_SCRIPT)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    module.main([])
