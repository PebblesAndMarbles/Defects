"""
probe_generic_description_v7.py
--------------------------------
v7 of the SMALL_PARTICLE generic-description pilot, built from
Particle_Descriptors_v7_canonical.txt:
...
3. Fine texture renamed for internal consistency: texture_layered ->
   texture_interior_layer (comparator-edge-following trigger intentionally
   dropped, defect's-own-outline-following only), texture_interior_striation ->
   texture_interior_line, texture_fractured -> texture_interior_fracture
   (dropped the "1-3 parts" bound down to "multiple pieces").
...
Uses the SAME pilot manifest as v1-v6 (not a fresh sample) so all runs are
...
def _call_with_retry(bright_path: Path, dark_path: Path, model: str) -> tuple[dict[str, Any], str, dict[str, Any], int]:
    parsed, raw_text, usage = _call_image(
        [bright_path, dark_path], GENERIC_DESCRIPTION_PROMPT_V7, model, DEFAULT_MAX_TOKENS
    )
    if _is_parsed_ok(parsed):
        return parsed, raw_text, usage, DEFAULT_MAX_TOKENS

    parsed, raw_text, usage = _call_image(
        [bright_path, dark_path], GENERIC_DESCRIPTION_PROMPT_V7, model, RETRY_MAX_TOKENS
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the v7 generic-description VLM pilot (fine-shape terms trimmed, coarse_shape/location_relative hardened, texture fields renamed) over the SMALL_PARTICLE sample manifest."
    )
    parser.add_argument("--pilot-manifest-csv", default=DEFAULT_PILOT_MANIFEST)
    parser.add_argument("--output-jsonl", default=None, help="Default: outputs/probes/generic_description_v7_<UTC timestamp>.jsonl")
...
def main() -> int:
    ...
    output_jsonl = Path(args.output_jsonl) if args.output_jsonl else (
        pilot_manifest_csv.parent / f"generic_description_v7_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.jsonl"
    )
    ...
    print(f"prompt_char_count={len(GENERIC_DESCRIPTION_PROMPT_V7)}")
