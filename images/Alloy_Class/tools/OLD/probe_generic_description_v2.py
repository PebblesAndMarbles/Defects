"""
probe_generic_description_v2.py
--------------------------------
v2 of the SMALL_PARTICLE generic-description pilot, built from user feedback on the
v1 run (outputs/probes/generic_description_v1_20260830T222707Z.jsonl, reviewed via
outputs/probes/generic_description_v1_pilot_review.html):

1. v1's free-text description overused "bright" (40/40 cases) and redundantly noted
   comparator/trench darkness (a constant of this imaging modality, not an observation).
   v2 explicitly forbids both in the prompt and shrinks description to 1-2 sentences.
2. v1's shape_descriptor/texture_descriptor were open free-text (e.g. "lobed" -- unclear
   jargon, and inconsistent vocabulary across cases is unusable for clustering). v2
   replaces them with a closed vocabulary: single-select coarse_shape/coarse_texture plus
   independent boolean fine-grained flags (see Particle_Descriptors copy.txt for the
   user-authored spec this is built from), designed to merge directly into production
   CSVs as True/False columns for correlation against process/module data.
3. v1's size_relative (small/medium/large) was a subjective bucket with no reference
   scale. v2 asks for a quantitative size_percent_of_image estimate instead; binning
   into categories is deferred until the distribution across a larger sample is known.
4. defect_count is known to sometimes undercount fused/multi-lobed particle clusters
   (v1 finding) -- v2 adds coarse_shape=clumped as an explicit hedge for this case.
5. New Comparator_Fit field (yes/comparable/no) per user spec.

Uses the SAME pilot manifest as v1 (not a fresh sample) so the runs are directly
apples-to-apples comparable on the same 40 cases -- same convention as
tools/probe_beep_lexicon_v2.py vs v1.
"""
...
GENERIC_DESCRIPTION_PROMPT_V2 = (
    ...
    "Do not comment on brightness/contrast of the defect or comparators, or on comparator "
    "color/darkness -- these are constant properties of this imaging modality, not "
    "meaningful observations.\n\n"
    "One or more material defects (residual material) may rest on the SiO surface, "
    "inside a comparator, or both. Look carefully for more than one distinct defect -- a "
    "single fused, multi-lobed blob may actually be several particles merged together; if "
    "so, still give your best defect_count but also set coarse_shape to clumped.\n\n"
    "OUTPUT CONTRACT: strict JSON only, no text outside the JSON object. Use exactly "
    "these keys in this order:\n"
    "coarse_shape (single value: circle, clumped, angular, round)\n"
    ...
    "location_relative (single value: on_sio, on_comparator, spanning_both, unclear)\n"
...
def _call_with_retry(bright_path: Path, dark_path: Path, model: str) -> tuple[dict[str, Any], str, dict[str, Any], int]:
    parsed, raw_text, usage = _call_image(
        [bright_path, dark_path], GENERIC_DESCRIPTION_PROMPT_V2, model, DEFAULT_MAX_TOKENS
    )
    if _is_parsed_ok(parsed):
        return parsed, raw_text, usage, DEFAULT_MAX_TOKENS

    parsed, raw_text, usage = _call_image(
        [bright_path, dark_path], GENERIC_DESCRIPTION_PROMPT_V2, model, RETRY_MAX_TOKENS
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the v2 generic-description VLM pilot (closed-vocab shape/texture) over the SMALL_PARTICLE sample manifest."
    )
    parser.add_argument("--pilot-manifest-csv", default=DEFAULT_PILOT_MANIFEST)
    parser.add_argument("--output-jsonl", default=None, help="Default: outputs/probes/generic_description_v2_<UTC timestamp>.jsonl")
...
def main() -> int:
    ...
    output_jsonl = Path(args.output_jsonl) if args.output_jsonl else (
        pilot_manifest_csv.parent / f"generic_description_v2_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.jsonl"
    )
    ...
    print(f"prompt_char_count={len(GENERIC_DESCRIPTION_PROMPT_V2)}")
