"""
v2 of the fresh lexicon-based probe, built from the user's second lexicon iteration
(images/Alloy_Class/BEEP_Evidence copy 2.txt, 2026-08-29) -- same single-call
architecture as tools/probe_beep_lexicon_v1.py, updated prompt text only, so this run is
directly apples-to-apples comparable against the v1 run on the same 31 cases.

Changes vs LEXICON_PROMPT_V1, translating the v2 lexicon edits (all driven by feedback
collected via the HTML feedback portal on the v1 run -- see
outputs/probes/scored/beep_lexicon_v1_20260828_full31/probe_review_feedback.csv):

1. COMPARATOR SHADOW DISQUALIFIER: a comparator's darkest region is explicitly called out
   as light-source geometry, never evidence -- targets BMK_0023/BMK_0032/BMK_0009's shadow
   mistaken for sunken residual.
2. INSET SURFACE LINES CONTINUITY REQUIREMENT: a genuine strip must be a continuous path
   from the SiO surface into the comparator interior; a defect that merely crosses the
   comparator boundary without that connecting strip does not confirm ISL -- targets
   BMK_0034.
3. BOUNDARY CONFORMANCE EXTERNAL-MATERIAL REQUIREMENT (both occluding and non-occluding):
   requires material defect present on the SiO surface outside the etched feature, on the
   far side of the conforming line; a defect entirely inside the comparator does not
   confirm conformance regardless of internal alignment -- targets BMK_0033.
4. SUNKEN RESIDUAL / SHADOW DECOUPLING: explicit statement that sunken residual is
   unrelated to a comparator's darkest (shadow) region.

See tools/probe_beep_lexicon_v1.py for the shared architecture notes (single call, no
separate Call-1 observation pass, output written in the Lineage B shape so the existing
tools/score_probe_run.py and reporting/build_probe_html_report.py work unmodified).
"""
...
LEXICON_PROMPT_V2 = (
    ...
    "depending on light-source position -- this alone is never evidence of anything. The darkest "
    "region within a comparator is simply light-source geometry and is never evidence of anything, "
    "no matter how dark it appears. Long dimensions of all comparators in an image are parallel to "
    "each other; positioning may be regular or irregular, in one or both dimensions. A line trench "
    "may terminate in a rounded cap in the frame, or span the full image height/width; short line "
    "trenches round-cap at both ends. Vias have a long and short side with rounded corners. Wide "
    "vias have a longer long-side than standard vias but a short side comparable to a via's, also "
    "rounded. Vias and wide vias may only partially land in the image frame.\n\n"
    ...
    "boundary. A genuine strip is a continuous path connecting the SiO surface to the comparator "
    "interior -- a defect boundary that merely crosses the comparator without such a connecting "
    "strip does NOT confirm inset surface lines. A strip extending far enough can bridge fully "
    "across a comparator. Evidence is strengthened when the strip's texture AND contrast both match "
    "the surrounding SiO substrate.\n\n"
    ...
    "line. The conforming line can be curved or mostly straight. Confirming this ALSO requires "
    "material defect present on the SiO surface, outside the etched feature, on the far side of the "
    "conforming line -- a defect entirely inside the comparator does NOT confirm conformance "
    ...
    "straight. Confirming this ALSO requires material defect present on the SiO surface, outside the "
    "etched feature, on the far side of the conforming line -- a defect entirely inside the "
    "comparator does NOT confirm conformance regardless of internal alignment.\n\n"
    "SUNKEN RESIDUAL MATERIAL: defect material inside a comparator that shares a conforming boundary "
    "(partial or full width) with defect material on the SiO surface. Can present as darker contrast "
    "than the SiO, but this is NOT related to a comparator's darkest region, which is simply "
    "light-source geometry, never residual material. Supporting evidence only -- use it to raise "
    "confidence when a conforming boundary is a near-but-imperfect match; never treat it as "
...
def _run_case(case: dict[str, str], model: str, max_completion_tokens: int) -> dict[str, Any]:
    ...
    parsed, raw_text, usage = _call_image([bf, df], LEXICON_PROMPT_V2, model, max_completion_tokens)
    ...
    return {
        ...
        "call1_observation": "",
        "call1_usage": None,
        "call2_prompt_chars": len(LEXICON_PROMPT_V2),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="v2 single-call probe built from the second BEEP_Evidence lexicon iteration."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output JSONL path (default: outputs/probes/beep_lexicon_v2_<UTC timestamp>.jsonl).",
    )
    ...
    if args.output:
        ...
        output_path = alloy_class_root / "outputs" / "probes" / f"beep_lexicon_v2_{stamp}.jsonl"
