"""
probe_generic_description_v5.py
--------------------------------
v5 of the SMALL_PARTICLE generic-description pilot, built from
Particle_Descriptors_v5_canonical.txt. Two changes from v4:

1. coarse_shape circle calibration reverted from "at most one subtle facet" back
   to the stricter "no facet or notch anywhere on the boundary" (v4's loosened
   wording increased circle calls 1->3 without a confirmed genuine circle in the
   set; user asked to revert).
2. texture_scraggly trigger definition shortened -- v4's longer definition (with
   the "no single identifiable break/fold" caveat) was over-called and missed a
   couple of times; user asked to go with a simpler definition instead.

Uses the SAME pilot manifest as v1/v2/v3/v4 (not a fresh sample) so all runs are
...
def _call_with_retry(bright_path: Path, dark_path: Path, model: str) -> tuple[dict[str, Any], str, dict[str, Any], int]:
    parsed, raw_text, usage = _call_image(
        [bright_path, dark_path], GENERIC_DESCRIPTION_PROMPT_V5, model, DEFAULT_MAX_TOKENS
    )
    if _is_parsed_ok(parsed):
        return parsed, raw_text, usage, DEFAULT_MAX_TOKENS

    parsed, raw_text, usage = _call_image(
        [bright_path, dark_path], GENERIC_DESCRIPTION_PROMPT_V5, model, RETRY_MAX_TOKENS
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the v5 generic-description VLM pilot (circle reverted to strict, scraggly shortened) over the SMALL_PARTICLE sample manifest."
    )
    parser.add_argument("--pilot-manifest-csv", default=DEFAULT_PILOT_MANIFEST)
    parser.add_argument("--output-jsonl", default=None, help="Default: outputs/probes/generic_description_v5_<UTC timestamp>.jsonl")
...
def main() -> int:
    ...
    output_jsonl = Path(args.output_jsonl) if args.output_jsonl else (
        pilot_manifest_csv.parent / f"generic_description_v5_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.jsonl"
    )
    ...
    print(f"prompt_char_count={len(GENERIC_DESCRIPTION_PROMPT_V5)}")
